...
static int block_size_to_nb(aes_block_size_t bs) {
    switch (bs) {
...
static int block_size_to_nr(aes_block_size_t bs) {
    switch (bs) {
...
void shift_rows(unsigned char *block, aes_block_size_t block_size) {
    ...
    memcpy(tmp, block, nb * 4);
 
    for (int row = 0; row < 4; row++) {
        for (int col = 0; col < nb; col++) {
            block[col * 4 + row] = tmp[((col + row) % nb) * 4 + row];
        }
    }
}
...
void invert_shift_rows(unsigned char *block, aes_block_size_t block_size) {
    ...
    for (int row = 0; row < 4; row++) {
        for (int col = 0; col < nb; col++) {
            block[col * 4 + row] = tmp[((col - row + nb) % nb) * 4 + row];
        }
    }
}
...
void mix_columns(unsigned char *block, aes_block_size_t block_size) {
    int nb = block_size_to_nb(block_size);
    for (int col = 0; col < nb; col++) {
        unsigned char *c = &block[col * 4];
        unsigned char s0=c[0], s1=c[1], s2=c[2], s3=c[3];
...
unsigned char *aes_encrypt_block(unsigned char *plaintext,
    ...
    unsigned char *rk = expand_key(key, block_size);
 
    add_round_key(state, &rk[0], block_size);
